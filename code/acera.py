...
class Ui_aceragui(object):
    ...
    def Load(self):
        ...
        if RUNNING == 0:
            ...
            if file_path != '':
                ...
                if width > 9999 or len(ids1) > 99:
                    self.error.setInformativeText(
                        'Sequences choosed exceeds model limits \n (Max sequences = 3 , Max length = 100).')
                    self.error.exec_()
                    self.statusbar.showMessage(
                        'Sequences choosed exceeds model limits (Max sequences = 3 , Max length = 100).')
                else:
                    ...
                    self.Scorelabel.setText(
                        "Alignment score:" + str(' ') + str(score) + "\n" + "Exact match: " + str(EMCount))
    ...
    def Align(self):
        ...
        if LOADED == 0:
            ...
            if STOP == 0:
                ...
                status = 'Sequences aligned in ' + \
                    str(round(TIME)) + ' seconds.'
    ...
    def Save(self):
        ...
        if ALIGNED == 0:
            ...
            if output_dir != '':
                ...
                o_file = path_leaf(file_path).split(".")
                tosave = 'Aligned with acera v' + \
                    str(__version__) + '.\n' + "\n" + "\n"
                for i in range(len(ids1)):
                    tosave += ids1[i].ljust(maxid + 6, ' ') + \
                        Alignment[i] + '\n'
                CSSTR = "".join(EM)
                tosave += ''.ljust(maxid + 6, ' ') + CSSTR
                with open(output_dir + "/" + o_file[0] + " - acera" + ".aln", "w") as text_file:
                    text_file.write(tosave)
                self.statusbar.showMessage(
                    'Alignment saved to:' + str(' ') + output_dir + "/" + o_file[0] + " - acera" + ".aln")
    ...
    def CheckUpdate(self):
        ...
        try:
            version = urlopen(
                'https://github.com/mbouskri/acera/blob/master/installer/version.txt?raw=true').read()
            if float(version) > float(__version__):
                url = 'https://github.com/mbouskri/acera/blob/master/installer/acera-installer.exe?raw=true'
                r = requests.get(url, allow_redirects=True)
                open('installer/acera-installer.exe', 'wb').write(r.content)
    ...
    def retranslateUi(self, aceragui):
        ...
        aceragui.setWindowTitle(_translate(
            "aceragui", "acera v" + str(__version__)))
        ...
        self.actionLoad_sequences.setText(
            _translate("aceragui", "Load sequences"))
        self.actionSave_alignment.setText(
            _translate("aceragui", "Save alignment"))
        self.actionExit.setText(_translate("aceragui", "Exit"))
        self.actionAlign_sequences.setText(
            _translate("aceragui", "Align sequences"))
        self.actionStop_alignment.setText(
            _translate("aceragui", "Stop alignment"))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if float(version) > float(__version__):
            ...
            app.processEvents()
            statusbar.setText('acera will close to install updates.')
            app.processEvents()
            time.sleep(1.5)
            win32api.ShellExecute(
                0, 'open', 'installer/acera-installer.exe', None, None, 10)
